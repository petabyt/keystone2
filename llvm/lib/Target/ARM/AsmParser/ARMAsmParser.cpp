...
int ARMAsmParser::tryParseShiftRegister(OperandVector &Operands)
{
  ...
  if (!PrevOp->isReg()) {
    Error(PrevOp->getStartLoc(), "shift must be of a register");
    return -1;
  }
...
ARMAsmParser::parseITCondCode(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseCoprocNumOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  if (Tok.isNot(AsmToken::Identifier))
...
/// number, the token is eaten and the operand is added to the operand list.
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseCoprocRegOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  if (Tok.isNot(AsmToken::Identifier))
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseCoprocOptionOperand(OperandVector &Operands, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
...
parseVectorLane(VectorLaneTy &LaneKind, unsigned &Index, SMLoc &EndLoc, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseVectorList(OperandVector &Operands, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
  ...
  if (Parser.getTok().is(AsmToken::Identifier)) {
    ...
    if (ARMMCRegisterClasses[ARM::DPRRegClassID].contains(Reg)) {
      OperandMatchResultTy Res = parseVectorLane(LaneKind, LaneIndex, E, ErrorCode);
  ...
  SMLoc E;
  if (parseVectorLane(LaneKind, LaneIndex, E, ErrorCode) != MatchOperand_Success)
    return MatchOperand_ParseFail;
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseMemBarrierOptOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  unsigned Opt;
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseInstSyncBarrierOptOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  unsigned Opt;
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseProcIFlagsOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  if (!Tok.is(AsmToken::Identifier)) 
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseMSRMaskOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  if (!Tok.is(AsmToken::Identifier))
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseBankedRegOperand(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
  const AsmToken &Tok = Parser.getTok();
  if (!Tok.is(AsmToken::Identifier))
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseSetEndImm(OperandVector &Operands, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
  const AsmToken &Tok = Parser.getTok();
  SMLoc S = Tok.getLoc();
...
///             n == 32 encoded as n == 0.
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseShifterImm(OperandVector &Operands, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
  const AsmToken &Tok = Parser.getTok();
  SMLoc S = Tok.getLoc();
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseRotImm(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
  const AsmToken &Tok = Parser.getTok();
  SMLoc S = Tok.getLoc();
  if (Tok.isNot(AsmToken::Identifier))
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseModImm(OperandVector &Operands, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseBitfield(OperandVector &Operands, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
  SMLoc S = Parser.getTok().getLoc();
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parsePostIdxReg(OperandVector &Operands, unsigned int &ErrorCode) {
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseAM3Offset(OperandVector &Operands, unsigned int &ErrorCode) {
  // Check for a post-index addressing register operand. Specifically:
  // am3offset := '+' register
  //              | '-' register
  //              | register
...
ARMAsmParser::OperandMatchResultTy
ARMAsmParser::parseFPImm(OperandVector &Operands, unsigned int &ErrorCode) {
  MCAsmParser &Parser = getParser();
...
/// of the mnemonic.
bool ARMAsmParser::parseOperand(OperandVector &Operands, StringRef Mnemonic, unsigned int &ErrorCode)
{
  MCAsmParser &Parser = getParser();
  SMLoc S, E;
                                    ...
                                    SMLoc NameLoc, OperandVector &Operands, unsigned int &ErrorCode)
{
  ...
  if (!CanAcceptCarrySet && CarrySetting) {
    Parser.eatToEndOfStatement();
    Error(NameLoc, "instruction '" + Mnemonic +
  ...
  if (ProcessorIMod) {
    ...
    Error(NameLoc, "instruction 'cps' requires effect for M-class");
    ErrorCode = KS_ERR_ASM_MNEMONICFAIL;
    return true;
  }
  ...
  while (Next != StringRef::npos) {
    ...
    if (ExtraToken == ".n" && !isThumb()) {
      ...
      Parser.eatToEndOfStatement();
      Error(Loc, "instruction with .n (narrow) qualifier not allowed in "
            "arm mode");
      ErrorCode = KS_ERR_ASM_MNEMONICFAIL;
      return true;
    }
                                           ...
                                           bool MatchingInlineAsm, unsigned int &ErrorCode, uint64_t &Address)
{
  ...
  case Match_MnemonicFail:
    ...
    ErrorCode = KS_ERR_ASM_ARM_MNEMONICFAIL;
    return true;
  case Match_RequiresNotITBlock:
    Error(IDLoc, "flag setting instruction only valid outside IT block");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  case Match_RequiresITBlock:
    Error(IDLoc, "instruction only valid inside IT block");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  case Match_RequiresV6:
    Error(IDLoc, "instruction variant requires ARMv6 or later");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  case Match_RequiresThumb2:
    Error(IDLoc, "instruction variant requires Thumb2");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  case Match_RequiresV8:
    Error(IDLoc, "instruction variant requires ARMv8 or later");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  case Match_ImmRange0_15: {
    SMLoc ErrorLoc = ((ARMOperand &)*Operands[ErrorInfo]).getStartLoc();
    if (ErrorLoc == SMLoc()) ErrorLoc = IDLoc;
    Error(ErrorLoc, "immediate operand must be in the range [0,15]");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  }
  case Match_ImmRange0_239: {
    SMLoc ErrorLoc = ((ARMOperand &)*Operands[ErrorInfo]).getStartLoc();
    if (ErrorLoc == SMLoc()) ErrorLoc = IDLoc;
    Error(ErrorLoc, "immediate operand must be in the range [0,239]");
    ErrorCode = KS_ERR_ASM_ARM_INVALIDOPERAND;
    return true;
  }
  ...
  case Match_AlignedMemoryRequires64or128or256:
  {
    ...
    switch (MatchResult) {
      ...
      case Match_AlignedMemoryRequiresNone:
      case Match_DupAlignedMemoryRequiresNone:
        Error(ErrorLoc, "alignment must be omitted");
        return true;
      case Match_AlignedMemoryRequires16:
      case Match_DupAlignedMemoryRequires16:
        Error(ErrorLoc, "alignment must be 16 or omitted");
        return true;
      case Match_AlignedMemoryRequires32:
      case Match_DupAlignedMemoryRequires32:
        Error(ErrorLoc, "alignment must be 32 or omitted");
        return true;
      case Match_AlignedMemoryRequires64:
      case Match_DupAlignedMemoryRequires64:
        Error(ErrorLoc, "alignment must be 64 or omitted");
        return true;
      case Match_AlignedMemoryRequires64or128:
      case Match_DupAlignedMemoryRequires64or128:
        Error(ErrorLoc, "alignment must be 64, 128 or omitted");
        return true;
      case Match_AlignedMemoryRequires64or128or256:
        Error(ErrorLoc, "alignment must be 64, 128, 256 or omitted");
        return true;
    }
  }
  }
